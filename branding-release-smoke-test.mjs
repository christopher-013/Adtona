...
assert.match(versionSource, /ADTONA_VERSION\s*=\s*"5.6.2"/);
assert.match(versionSource, /PLANTOGUIDE_VERSION\s*=\s*globalThis\.ADTONA_VERSION/);
assert.equal(packageJson.version, "5.6.2");
assert.match(serverSource, /McpServer\(\{ name: "plantoguide", version: "5.6.2" \}\)/);
